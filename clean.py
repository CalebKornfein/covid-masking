...
def add_binary_masking_behaviors(vals):
    ...
    for val in vals:
        if val == 'Mask_Indoors_Always' or val == 'Mask_Indoors_Majority' or val == 'Mask_Indoors_Half':
...
def valid(df):
    return df.apply(lambda row: not(no_response(row, kMaskFrequencyOptions) or multiple_responses(row, kMaskFrequencyOptions)), axis=1)

def answered_masking(df):
    return df.apply(lambda row: not(row['Mask_Indoors_Unsure'] == 'yes' or row['Mask_Indoors_Do_Not_Go_Out'] == 'yes'), axis=1) 
...
def main():
    ...
    process(df, output_path='data/processed_full.csv', summary_path='results/summary_full.csv')

    # Also look at only infrequent mask users
    infrequent = df[df.apply(lambda row: row['Mask_Indoors_Infrequent'] == 'yes' or row['Mask_Indoors_Never'] == 'yes', axis=1)]
    process(infrequent, output_path='data/processed_infrequent.csv', summary_path='results/summary_infrequent.csv')

    # And frequent mask users
    frequent = df[df.apply(lambda row: row['Mask_Indoors_Always'] == 'yes' or row['Mask_Indoors_Majority'] == 'yes' or row['Mask_Indoors_Half'] == 'yes', axis=1)]
    process(frequent, output_path='data/processed_frequent.csv', summary_path='results/summary_frequent.csv')
