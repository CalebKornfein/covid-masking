...
def create_confints(df, frequent, infrequent, path):
        ...
        for c in ordering:
            ...
            rows.append(
                [
                    ...
                    min(
                        round(frequent_percent + length, 2), 100
                    ),  # frequent upper bound 95% CI
                    max(
                        round(frequent_percent - length, 2), 0
                    ),  # frequent lower bound 95% CI
                    infrequent_counter[c],  # infrequent count
                    round(infrequent_percent, 2),  # infrequent percentage
                    min(
                        round(infrequent_percent + length, 2), 100
                    ),  # infrequent percentage upper
                    max(
                        round(infrequent_percent - length, 2), 0
                    ),  # infrequent percentage lower
                ]
            )
        return rows
    ...
    cols = [
        ...
        "Frequent Count",
        "Frequent Percentage",
        "Frequent Upper Bound",
        "Frequent Lower Bound",
        "Infrequent Count",
        "Infrequent Percentage",
        "Infrequent Upper Bound",
        "Infrequent Lower Bound",
    ]
    final.columns = cols
